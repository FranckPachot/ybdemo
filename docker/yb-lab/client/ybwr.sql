...
# this purges to the snaps to keep only the last 100 snapshots
delete from ybwr_snapshots where ts not in (select distinct ts from ybwr_snapshots order by ts desc limit 100);
# example:
select ybwr_snap();
select ybwr_snap();
